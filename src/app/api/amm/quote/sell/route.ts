...
// GET /api/amm/quote/sell?amount=<tokenAmount>
// Returns the proceeds from selling a given amount of tokens
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (totalSupply < tokenAmount) {
      return NextResponse.json({
        error: "Insufficient supply for this sell amount",
        totalSupply: totalSupply.toString(),
        requested: tokenAmount.toString(),
      }, { status: 400 });
    }

    // Calculate sell proceeds using bonding curve integration
    // Proceeds = initialPrice × amount + slope × (S1² - S2²) / 2
    const SCALE = BigInt(1_000_000_000);
    const slopeNumerator = initialPrice * reserveRatio;

    // Linear proceeds: initialPrice × amount
    const linearProceeds = initialPrice * tokenAmount;

    // Quadratic proceeds: slope × (S1² - S2²) / 2
    const s1 = totalSupply;
    const s2 = totalSupply - tokenAmount;
    const s1Squared = s1 * s1;
    const s2Squared = s2 * s2;
    const diffSquared = s1Squared - s2Squared;

    // quadraticProceeds = slopeNumerator × diffSquared / (20000 × SCALE)
    const quadraticProceeds = (slopeNumerator * diffSquared) / (BigInt(20000) * SCALE);

    const totalProceedsMotes = linearProceeds + quadraticProceeds;
    const totalProceedsCSPR = Number(totalProceedsMotes) / 1_000_000_000;
    ...
    const pricePerToken = totalProceedsCSPR / Number(tokenAmount);
    ...
    return NextResponse.json({
      ...
      estimatedProceeds: {
        motes: totalProceedsMotes.toString(),
        cspr: totalProceedsCSPR,
      },
      pricePerToken,
    ...
    console.error("AMM sell quote error:", error);
